...
def main():
    args = parse_args()
    w3 = connect(args.rpc)
  print(f"📝 Proof cost audit started at: {time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime())} UTC")
print(f"💼 Contracting network: {network_name(int(w3.eth.chain_id))} (chainId {w3.eth.chain_id})")
